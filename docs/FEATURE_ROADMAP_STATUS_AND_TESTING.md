...
| ----- | ---------------------------------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | **Team accounts for businesses**         | Active            | Owner-managed business teams are active with roles and invites. Built-in roles are seeded on first use, owners can create custom roles, and custom-role deletion requires member reassignment.                                                                                                                                                                                                                                                                                                                 |
| **2** | **Email / SMS / WhatsApp notifications** | Done (email path) | **API:** When creating a notification, pass `recipientEmail` (and optional `recipientDisplayName`) in the payload; a transactional email is sent (Resend launch path, legacy Brevo/SendGrid optional) in addition to in-app. **Test:** Use admin “Send notification” or any code that calls `notificationsService.createForUser(userId, { ..., recipientEmail: 'test@example.com' })`. SMS/WhatsApp: add providers when ready; API is ready for fan-out.                                                       |
...
- [x] **Team accounts:** Owner-managed business teams are active with roles and invites; continue testing member permissions before heavy promotion.
