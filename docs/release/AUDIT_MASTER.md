...
| PLT-04 | Auth | **LIVE BUG — email change was broken.** `users.pending_email`, `pending_email_token`, `pending_email_expires` are read/written by `auth.repository.ts` and cleared by `admin.repository.ts`, but did not exist in the database. Every email-change attempt failed. Cause: version collision — the old runner applied `006_future_proof_fields`, then CLI baselining marked `20240101000006` (a *different* 006, pending_email) as applied, so it was skipped forever. `pending_email_attempts` existed because a later migration added it, which is why nothing noticed. | 2 | **Fixed** | No | `8e73324` | replay check (regression guard) | **V** — `SELECT pending_email` failed on live |
| MHC-28 | Migrations | 7 migrations applied but absent from the repo. **Recovered verbatim** from `supabase_migrations.schema_migrations.statements` — originals, not reconstructions. A second drift layer was also found and fixed: commit `5e04ed1` rewrote rather than copied the legacy numbered migrations and dropped `006_future_proof_fields.sql` entirely. | 3 | **Fixed** | No | `b82dde5` | `scripts/migration-replay-check.mjs` | **V** — replay now matches live exactly |
| MHC-26 | Migrations | ~~Neither MHC migration is applied to the dev database.~~ **Resolved** — all 5 pending migrations applied to dev after backup + dry run; 20/20 validations pass; 94 migrations recorded. | 1 | **Fixed** | D1 answered | `8e73324` | 20 post-apply validations | **V** — validation output |
| ~~MHC-26~~ | ~~Migrations~~ | *(superseded by the row above)* **Neither MHC migration is applied to the dev database.** No `mhc_*` tables; `wallets` has no `account_type`/`asset_code`; `needs` has no pending-award columns; `needs_status_check` and `bids_status_check` are still the pre-MHC sets. The `f7fda17` API code therefore cannot run against this database at all — every wallet read filters on a non-existent column. | 1 | Open | No (needs confirmation to apply) | — | — | **V** — read-only DB inspection, 2026-07-28 |
...
| MHC-29 | Migrations | `deposit_requests` carries two status CHECKs; both are enforced for new rows, so the **effective** `status` domain is their intersection: `pending, paid, expired, failed, cancelled, pending_review, rejected`. `'completed'` is unreachable — code testing for it was dead. Corrected from an earlier note: this does **not** block the underpayment policy, because the existing NOWPayments rail records `'underpaid'` in `provider_status` (free-text, no CHECK), not in `status`. | 5 | Documented | No | `step 4` | encoded in `FULFILLABLE_PURCHASE_STATUSES` doc comment | **V** — `pg_constraint` dump + `wallet.repository.updateDepositProviderStateByOrderId` |
| MHC-12 | Purchases | `fulfillCreditPurchase` granted from any status outside paid/completed/rejected/cancelled — including `expired`, `failed`, `underpaid`. Webhook path never validated that the provider status meant "settled". Now an explicit allow-list on both sides: fulfilment only from `pending`/`pending_review`, webhooks only from `finished`. Non-settled callbacks are recorded for audit without moving credits. Admin `overrideMhcAmount` of 0 or negative is refused (it previously marked the purchase paid while granting nothing, or debited the provider). | 3 | **Fixed** | No | `step 4` | 20 new tests incl. status matrices | **V** — `mhc.repository.ts`, `mhc.service.ts` |
