...
- Offers are D0 (publicly visible including to guests) unless the offer sets a buyer
  verification requirement, in which case the _request action_ is gated, not the listing.
...
| `paused`         | No             | Yes, marked _not currently accepting_, prices shown                 | No           | **Unaffected**       | Owner                                                         |
...
| `archived`       | No             | Yes, read-only, marked _no longer offered_; reviews remain readable | No           | **Unaffected**       | Owner                                                         |

### 8.3 Rules

- **No state change ever touches a live engagement.** Pausing, hiding, rejecting or archiving
  an Offer changes discovery and the ability to create _new_ arrangements. Everything already
