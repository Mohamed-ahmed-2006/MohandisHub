-- ============================================================================
-- MohandisHub Auth Schema â€” v001
-- Designed for a Vezeeta-style engineering marketplace with 3 roles:
--   customer  â†’ requests services / posts jobs
--   expert    â†’ freelance engineer, requires identity verification
--   business  â†’ company profile, requires business verification
-- ============================================================================
...
-- --------------------------------------------------------------------------
-- 1. Core users table â€” role-agnostic identity
-- --------------------------------------------------------------------------
CREATE TABLE users (
...
-- --------------------------------------------------------------------------
-- 2. Refresh tokens â€” session management with token rotation
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 4. Expert profiles â€” individual engineers / freelancers
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 5. Business profiles â€” companies
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 6. Verification requests â€” audit trail for KYC / KYB
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub OTP Schema â€” v002
-- 6-digit verification codes for email and phone
-- ============================================================================

-- --------------------------------------------------------------------------
-- 1. Verification codes table â€” stores OTP codes with rate-limiting support
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 2. Rate-limit table â€” prevents OTP spam (per user + channel)
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub â€” v003: Add date_of_birth to users
...
-- ============================================================================
-- MohandisHub â€” v004: Enhanced profiles, identity documents, academic records
--
-- SPLITS verification into two tracks:
--   1. Identity verification  â†’  national ID / driving license / passport
--      (can be auto-verified via Idenfy or manual admin review)
--   2. Academic / career verification  â†’  degrees, certificates
...
-- --------------------------------------------------------------------------
-- 1. Identity documents â€” national ID, driving license, passport
...
-- --------------------------------------------------------------------------
-- 2. Academic records â€” degrees, certifications
...
ALTER TABLE business_profiles
  ...
  ADD COLUMN commercial_register  VARCHAR(100),           -- Ø³Ø¬Ù„ ØªØ¬Ø§Ø±ÙŠ
...
-- --------------------------------------------------------------------------
-- 5. Admin reviews â€” unified audit trail for any verification decision
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub â€” v005: Add missing columns referenced by auth repository
--
...
-- --------------------------------------------------------------------------
-- 1. Plans table â€” subscription / tier lookup
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub â€” v006: Add pending email change support
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v007: Expand plans table with pricing, billing, and features
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v008: Wallets and transactions
-- ============================================================================

-- --------------------------------------------------------------------------
-- 1. Wallets â€” one per user
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 2. Transactions â€” ledger of all wallet movements
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub â€” v009: Service categories (with i18n columns) and services
-- ============================================================================

-- --------------------------------------------------------------------------
-- 1. Service categories â€” admin-managed, bilingual
-- --------------------------------------------------------------------------
...
INSERT INTO service_categories (name_en, name_ar, slug, icon, sort_order) VALUES
  ('Structural Engineering',  'Ø§Ù„Ù‡Ù†Ø¯Ø³Ø© Ø§Ù„Ø¥Ù†Ø´Ø§Ø¦ÙŠØ©',   'structural-engineering',  'building',    1),
  ('Electrical Engineering',  'Ø§Ù„Ù‡Ù†Ø¯Ø³Ø© Ø§Ù„ÙƒÙ‡Ø±Ø¨Ø§Ø¦ÙŠØ©',   'electrical-engineering',  'zap',         2),
  ('Civil Engineering',       'Ø§Ù„Ù‡Ù†Ø¯Ø³Ø© Ø§Ù„Ù…Ø¯Ù†ÙŠØ©',      'civil-engineering',       'road',        3),
  ('Mechanical Engineering',  'Ø§Ù„Ù‡Ù†Ø¯Ø³Ø© Ø§Ù„Ù…ÙŠÙƒØ§Ù†ÙŠÙƒÙŠØ©',  'mechanical-engineering',  'cog',         4),
  ('Architectural Design',    'Ø§Ù„ØªØµÙ…ÙŠÙ… Ø§Ù„Ù…Ø¹Ù…Ø§Ø±ÙŠ',     'architectural-design',    'drafting',    5),
  ('Consultation',            'Ø§Ø³ØªØ´Ø§Ø±Ø©',              'consultation',            'message',     6),
  ('Site Visit',              'Ø²ÙŠØ§Ø±Ø© Ù…ÙˆÙ‚Ø¹',           'site-visit',              'map-pin',     7),
  ('Inspection',              'ÙØ­Øµ ÙˆÙ…Ø¹Ø§ÙŠÙ†Ø©',          'inspection',              'search',      8),
  ('Project Management',      'Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø´Ø§Ø±ÙŠØ¹',       'project-management',      'clipboard',   9),
  ('HVAC',                    'ØªÙƒÙŠÙŠÙ ÙˆØªØ¨Ø±ÙŠØ¯',         'hvac',                    'thermometer', 10),
  ('Plumbing',                'Ø§Ù„Ø³Ø¨Ø§ÙƒØ©',              'plumbing',                'droplet',     11),
  ('Surveying',               'Ø§Ù„Ù…Ø³Ø§Ø­Ø©',              'surveying',               'compass',     12)
ON CONFLICT (slug) DO NOTHING;

-- --------------------------------------------------------------------------
-- 2. Services â€” provider offerings
-- --------------------------------------------------------------------------
...
-- ============================================================================
-- MohandisHub â€” v010: Deposit requests (for Cryptomus / crypto)
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v011: Password reset token fields
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v014: is_admin flag + plans backfill
--
...
-- ============================================================================
-- MohandisHub â€” v019: Commission settings for bid/booking payments
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v020: Bid payment flow (wallet debit/credit + commission)
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v021: Bookings table (service/need bookings)
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v022: Availability slots for provider calendar
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v023: Reviews and ratings
-- ============================================================================
...
-- ============================================================================
-- MohandisHub â€” v024: Add 'completed' status to needs for order lifecycle
-- ============================================================================
...
-- Business onboarding completion milestone â€” backend-backed so completion is persisted
ALTER TABLE business_profiles
...
-- ============================================================================
-- MohandisHub â€” EGP-primary wallet, InstaPay manual rails, FX settings, USDâ†’EGP migration
-- ============================================================================
...
-- --------------------------------------------------------------------------
-- 3) deposit_requests â€” manual InstaPay + review + FX snapshot
-- --------------------------------------------------------------------------
ALTER TABLE deposit_requests
...
-- --------------------------------------------------------------------------
-- 4) withdrawal_requests â€” EGP source + crypto payout amount + InstaPay + proof
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 5) One-time USD â†’ EGP migration (wallets + transactions linked to USD wallets)
-- --------------------------------------------------------------------------
DO $$
DECLARE
  r NUMERIC(18, 6);
...
BEGIN
  ...
  IF settings_count = 0 THEN
    RAISE NOTICE 'app_settings empty; skip USDâ†’EGP migration';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'wallet USDâ†’EGP migration applied at rate %', r;
END $$;
...
-- ============================================================================
-- MohandisHub â€” Plans: which primary roles may see and subscribe to each plan
-- ============================================================================
...
SELECT 'Starter 7 Days', 'Ø®Ø·Ø© 7 Ø£ÙŠØ§Ù…', 7, 150, 'EGP', 'Basic promotion for one week', 'Ø¥Ø¹Ù„Ø§Ù† Ø£Ø³Ø§Ø³ÙŠ Ù„Ù…Ø¯Ø© Ø£Ø³Ø¨ÙˆØ¹'
