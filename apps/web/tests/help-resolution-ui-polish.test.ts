...
describe('Wave 2I Help & Resolution UI Polish Tests', () => {
  ...
  describe('Desktop Master/Detail & Mobile Navigation Structure', () => {
    ...
    it('supports 375px mobile breakpoint without horizontal overflow', () => {
      ...
      expect(screenCss).toContain('max-width: min(620px, calc(100vw - 2rem))');
  ...
  describe('Modal Dialog Accessibility & Unsupported Creation States', () => {
    it('implements modal semantics, keyboard focus trapping, Escape, and focus restoration', () => {
      const screenSource = readFileSync(
        new URL('../components/app/help-resolution-screen.tsx', import.meta.url),
        'utf8',
      );
  ...
  describe('Safety-report confidentiality', () => {
    it('never renders a counterparty for a safety report', () => {
      const screenSource = readFileSync(
        new URL('../components/app/help-resolution-screen.tsx', import.meta.url),
        'utf8',
      );
      expect(screenSource).toContain("item.kind !== 'safety_report' && item.counterpartyName");
