    ...
    return (
      <Container>
        <div style={{ padding: '2rem 0', textAlign: 'center' }}>
          <p>{isAr ? 'هذه الصفحة مخصصة لمزودي الخدمات فقط.' : 'Analytics is available for providers only.'}</p>
          <Link href={buildLocalePath(locale, '/app')} style={{ color: '#0f766e', fontWeight: 600 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
            ...
            <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '1.25rem' }}>
              ...
              <p style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#9ca3af' }}>
                {analytics.conversion?.orders ?? 0} {isAr ? 'طلبات' : 'orders'} / {analytics.conversion?.views ?? 0}{' '}
                {isAr ? 'مشاهدات' : 'views'}
              </p>
            </div>

            {analytics.topServices?.length > 0 && (
              <div style={{ gridColumn: '1 / -1', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '1.25rem' }}>
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {analytics.topServices.map((svc) => (
                    <li key={svc.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f3f4f6', paddingBottom: '0.5rem' }}>
                      <span style={{ fontWeight: 500 }}>{svc.title}</span>
                      <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>
                        {svc.viewCount} {isAr ? 'مشاهدة' : 'views'} · {svc.orderCount} {isAr ? 'طلب' : 'orders'}
                      </span>
                    </li>
                  ))}
                </ul>
